...
subroutine ca_initdata(level,time,lo,hi,nscal, &
  ...
  real(rt) :: xdist,x,r,upert(2),shear_height_r,velocity_gradient
  integer i,j,k,n,vortex,int_shear_width_x,int_shear_width_y,shear_bottom_index

  type (eos_t) :: eos_state

  shear_height_r = shear_height * delta(1)
  ...
  do i = lo(1), hi(1)
     ...
     state(i,URHO)  = interpolate(x,npts_model,model_r, &
          model_state(:,idens_model))
     state(i,UTEMP) = interpolate(x,npts_model,model_r, &
          model_state(:,itemp_model))

     
     do n = 1, nspec
        state(i,UFS-1+n) = interpolate(x,npts_model,model_r, &
             model_state(:,ispec_model-1+n))
     enddo

     eos_state%rho = state(i,URHO)
     eos_state%T = state(i,UTEMP)
     eos_state%xn(:) = state(i,UFS:)
